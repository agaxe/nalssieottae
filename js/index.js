...
function checkLocation() {
  ...
  function loadingIndicator() {
    ...
    const loadingTitleFirstEl =
      loadingTitleListEl.firstElementChild.cloneNode(true);
  ...
  if (location) {
    ...
    function weatherAPI() {
      ...
      fetch(url)
        ...
        .then(res => {
          ...
          weekData.map(item => {
            ...
            weekWeatherEl.innerHTML += `
                        <li>
                            ...
                            <div class="week-temp">${Math.round(
                              temp.day
                            )}°</div>
                        </li>
                        `;
          ...
          innerDataText(
            document.querySelector('.low-temp'),
            Math.round(daily[0].temp.min)
          );
          innerDataText(
            document.querySelector('.high-temp'),
    ...
    const searchLocationInputEl = searchLocationEl.querySelector(
      ...
      fetch(url, {
        ...
        .then(res => {
          if (res.documents.length) {
            ...
            locationList.innerHTML =
              '<li class="location-list-help">검색결과가 없습니다</li>';
          }
        })
        .catch(err => console.log(err));
    }
